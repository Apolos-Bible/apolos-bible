...
export function SyncIndicator() {
  ...
  const label = state === 'saving' ? 'Guardando…' : state === 'saved' ? 'Sincronizado' : state === 'error' ? error || 'Error al sincronizar' : pending ? `Sin conexión · ${pending} pendientes` : 'Sin conexión'
