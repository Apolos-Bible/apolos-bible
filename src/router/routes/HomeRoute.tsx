...
export function HomeRoute() {
  ...
  const reading = data?.last_reading
  const openReading = (withCommentary = false) => {
    if (!reading?.book_slug) return
    const passage = paths.bible({ lang: 'es', book: reading.book_slug, chapter: reading.chapter, verse: reading.verse })
    navigate(withCommentary ? `${passage}?commentary=1` : passage)
  }
      ...
      {!data && !error ? <div role="status" className="mt-8 text-sm text-text-muted">Preparando tu día…</div> : data && <>
        <section className="relative mt-6 overflow-hidden rounded-2xl border border-border-subtle bg-bg-secondary">
          ...
          <div className="relative grid min-h-[260px] md:grid-cols-[minmax(0,1fr)_240px]">
            <div className={`flex flex-col items-start justify-between p-6 text-left md:p-8 ${!reading ? 'opacity-60' : ''}`}>
              ...
              <div className="my-8"><p className="text-sm text-text-muted">Tu lectura</p><h2 className="mt-2 font-reading text-3xl leading-tight text-text-primary md:text-4xl">{reading ? `${reading.book_name ?? reading.book_slug} ${reading.chapter}:${reading.verse}` : 'Empieza hoy por Génesis'}</h2>{reading?.version && <p className="mt-2 text-xs uppercase tracking-wider text-text-muted">{reading.version}</p>}{commentaryPreview && <><p className="mt-5 text-2xs font-medium uppercase tracking-wider text-text-muted">Comentario bíblico</p><p className="mt-2 line-clamp-4 max-w-2xl font-reading text-sm leading-6 text-text-secondary">{commentaryPreview}</p></>}</div>
              <div className="flex flex-wrap gap-3">
                <button type="button" onClick={() => openReading()} disabled={!reading} className="inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2.5 text-sm font-semibold text-bg-primary transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"><BookOpen className="h-4 w-4" />Continuar lectura</button>
                <button type="button" onClick={() => openReading(true)} disabled={!reading} className="group inline-flex items-center gap-2 rounded-lg border border-border-subtle bg-bg-primary/60 px-4 py-2.5 text-sm font-semibold text-text-primary transition-colors hover:border-accent/50 hover:text-accent disabled:cursor-not-allowed disabled:opacity-50"><MessageCircle className="h-4 w-4" />Leer con comentario <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></button>
              </div>
            </div>
