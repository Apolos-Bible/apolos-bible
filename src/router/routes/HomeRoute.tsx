      ...
      {!data && !error ? <div role="status" className="mt-8 text-sm text-text-muted">Preparando tu día…</div> : data && <>
        <section className="relative mt-6 overflow-hidden rounded-2xl border border-border-subtle bg-bg-secondary">
          ...
          <div className="relative grid min-h-[260px] md:grid-cols-[minmax(0,1fr)_240px]">
            <button onClick={() => openReading(true)} disabled={!reading} className="group flex flex-col items-start justify-between p-6 text-left md:p-8 disabled:opacity-60">
              ...
              <div className="my-8"><p className="text-sm text-text-muted">Tu lectura</p><h2 className="mt-2 font-reading text-3xl leading-tight text-text-primary md:text-4xl">{reading ? `${reading.book_name ?? reading.book_slug} ${reading.chapter}:${reading.verse}` : 'Empieza hoy por Génesis'}</h2>{reading?.version && <p className="mt-2 text-xs uppercase tracking-wider text-text-muted">{reading.version}</p>}{commentaryPreview && <><p className="mt-5 line-clamp-4 max-w-2xl font-reading text-sm leading-6 text-text-secondary">{commentaryPreview}</p><p className="mt-2 text-2xs font-medium uppercase tracking-wider text-text-muted">Comentario bíblico</p></>}</div>
              <span className="inline-flex items-center gap-2 text-sm font-semibold text-accent">Leer más <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
            </button>
